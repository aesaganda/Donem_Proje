...
// Kullanýlan deðiþkenler
int kartNo[100];
int fisIslemSayaci = 0;
float karttakiPara[100];
float kasadakiPara; // test amaçlý 1000 deðeri
...
struct musteriBilgileri
{
    ...
    char *kartNumarasi;
    char *gecerlilikTarihi; // tarih ay/yýl seklinde
    char *kartSahibiAdi;
    char *guvenlikKodu;
...
// Ekleme ve Güncelleme Fonksiyonlarýi
...
// test tipinde deðiþken oluþturulmasý
...
void icecekSecimVeFiyatlandirma(struct menuBilgi *ptrYemek_Menu, int n)
{
    ...
    case 0:
        ...
        if (tercih)
        {
            ...
            Sleep(1100);
            kasadakiPara = Oku();        //kasadakiPara dosyadan okunuyor son yazan bakiye kasadakiPara degiskenine atiliyor
            kasadakiPara += toplamTutar; //islem gerceklesti
            Yaz(kasadakiPara);           //kasadakiPara degiskenindeki deger dosyaya yaziliyor
            toplamTutar = 0;
            odemeYontemi();
        }
        else
    ...
    case 1:
        ...
        kasadakiPara += toplamTutar; //islem gerceklesti
        Yaz(kasadakiPara);           //kasadakiPara degiskenindeki deger dosyaya yaziliyor
        //! fis yazdýrma isleminde icecek ile fisin sonlanacagý if ile düzenlenmeli
        toplamTutar = 0;
        break;
...
void siparisDetaylari(float *fiyatToplam, float tekFiyat, char isim[], int fisIslemSayaci)
{
    ...
    *fiyatToplam += istenenSayi * tekFiyat;
    ...
    if (fisIslemSayaci == 0) // TODO: 0 bilgisi yani ilk siparis verilince yeni dosya acilacak fis tasarýmý baslayacak //cerceve vs
    {
    ...
    if (fisIslemSayaci == 3) //TODO: 3 bilgisi yani son siparis verildigi ve mevcut dosyanin kapatilmasi ve fis bitme tasarýmý //cerceve vs
    {
...
//Gerekli if yerlerinde kullanýlmak uzere secim icin fonksiyon
int secimFonksiyonu()
{
    int secim;
    echo("==> ");
