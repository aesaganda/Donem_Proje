...
void ana_menu(struct menuBilgi *x)
{
    cls();
    int i, n = 0;
    char anaMenuListesi[3][20] = {"Yemek Siparisi", "Admin Paneli", "Cikis"};
    tab_bas(12);
...
void siparisDetaylari(float *fiyatToplam, float tekFiyat, char isim[], int fisIslemSayaci)
{
    ...
    *fiyatToplam += istenenSayi * tekFiyat;
    ...
    if (fisIslemSayaci == 3) //3 bilgisi yani son siparis verildigi ve mevcut dosyanin kapatilmasi
    {
        FILE *fSiparisFis = fopen("siparisFis.txt", "a");
        if (fSiparisFis == NULL)
        {
...
void odemeYontemi(struct menuBilgi *x)
{
    ...
    int i, n = 0;
    char odeme[3][30] = {"Online Odeme", "Kapida Odeme", "Siparis Iptali"};
    satir_bas(10);
    for (i = 0; i < 3; i++)
    {
        satir_bas(2);
