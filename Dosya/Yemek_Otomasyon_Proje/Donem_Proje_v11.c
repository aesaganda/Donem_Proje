...
void yeniMusteriOlustur(struct musteriBilgileri *y, struct menuBilgi *x)
{
    ...
    FILE *fpp;
    fpp = fopen("musteriGirisiBilgisi.tsv", "r");
    ...
    Sleep(200);
    satir_bas(3);
    //musteriGirisiBilgisi.tsv dosyasi islemleri yapilacak
    tab_bas(9);
    printf("E-posta:");
    ...
    FILE *Fp = fopen("musteriGirisiBilgisi.tsv", "a");
    if (Fp == NULL)
    {
        tab_bas(11);
        puts("!!! musteriGirisiBilgisi.tsv Dosya olusturulamadi !!!\n");
        Sleep(500);
        exit(1);
    }
    else
    ...
    //echo("musteriBilgileri.tsv dosyasi islemleri yapilacak.\n");
    ...
    FILE *Fpk = fopen("musteriBilgileri.tsv", "a");
    if (Fpk == NULL)
    {
        tab_bas(11);
        puts("!!! musteriBilgileri.tsv Dosya olusturulamadi !!!\n");
        Sleep(500);
        exit(1);
    }
    else
...
void mGirisiBilgisiCek(struct musteriBilgileri *y)
{
    ...
    FILE *fpp;
    if ((fpp = fopen("musteriGirisiBilgisi.tsv", "r")) != NULL)
    {
    ...
    {
        tab_bas(11);
        printf("!!!  musteriGirisiBilgisi.tsv Bulunamadi !!!");
        Sleep(1000);
    }
    fclose(fpp);
}
...
void mBilgiCek(struct musteriBilgileri *y)
{
    ...
    FILE *fpp;
    if ((fpp = fopen("musteriBilgileri.tsv", "r")) != NULL)
    {
    ...
    {
        tab_bas(11);
        printf("!!! musteriBilgileri.tsv Bulunamadi !!!");
        Sleep(1000);
    }
    fclose(fpp);
}
...
void icecekSecimVeFiyatlandirma(struct musteriBilgileri *y, struct menuBilgi *x, int n)
{
    FILE *Fp = fopen("kasadakiPara.tsv", "a");
    if (Fp == NULL)
    {
        tab_bas(8);
        puts(" kasadakiPara.tsv Dosya olusturulamadi.\n");
        Sleep(300);
        exit(1);
    }
    fclose(Fp);
...
void siparisDetaylari(float *fiyatToplam, float tekFiyat, char isim[], int fisIslemSayaci)
{
    ...
    *fiyatToplam += istenenSayi * tekFiyat;
    ...
    {
        FILE *fSiparisFis = fopen("siparisFis.tsv", "a");
        if (fSiparisFis == NULL)
        {
...
void musteriBilgileriGoruntule(struct musteriBilgileri *y, struct menuBilgi *x) //!Yazdirma kisimlari yanlis yazdiriyor.
{
    ...
    FILE *fpp;
    fpp = fopen("musteriBilgileri.tsv", "r");
    ...
    satirSayac = 0;
    FILE *fpk;
    fpk = fopen("musteriGirisiBilgisi.tsv", "r");
...
void siparisBilgileriGoruntule(struct musteriBilgileri *y, struct menuBilgi *x)
{
    ...
    FILE *siparisBilgileri = fopen("siparisFis.tsv", "r");
...
{
    int top;
    FILE *Fp = fopen("kasadakiPara.tsv", "r");
    if (Fp == NULL)
    {
        tab_bas(11);
        puts("!!! kasadakiPara.tsv Dosya okunamadi !!!\n");
        Sleep(500);
        exit(1);
    }
    else
